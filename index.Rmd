...
load("C:/Users/j.morawski/Documents/ignacymorawski/ignacymorawski.github.io/data/wse.RData")
load("C:/Users/j.morawski/Documents/ignacymorawski/ignacymorawski.github.io/data/nc.RData")

#TO ZROBIĆ NA KOŃCU 03.01.2023
...
save(ws1,ws2,file="C:/Users/j.morawski/Documents/ignacymorawski/ignacymorawski.github.io/data/wse.RData")
}
...
for (i in seq(length(lnce2))){
  ...
  download.file(paste0("https://newconnect.pl/pub/NEWCONNECT/statystyki/statystyki_dzienne/NC_",lnce3[i],".xls"),
                destfile=paste0("C:/Users/j.morawski/Documents/ignacymorawski/ignacymorawski.github.io/data/ncq/NC_",lnce3[i],".xls"),mode="wb")
    
  
  u0<-read_excel(paste0("C:/Users/j.morawski/Documents/ignacymorawski/ignacymorawski.github.io/data/ncq/NC_",lnce3[i],".xls"),3,skip=5,col_names = TRUE)
...
save(nc1,nc2,file="C:/Users/j.morawski/Documents/ignacymorawski/ignacymorawski.github.io/data/nc.RData")
}

#ind[7,2]<-"WIG.GAMES5"

load("C:/Users/j.morawski/Documents/ignacymorawski/ignacymorawski.github.io/data/sector.RData")
lsec<-tail(sector$date,1)
if (lsec<Sys.Date()){


lsec2<-format(seq(as.Date(lsec)+1,Sys.Date(),by="day"))


for (i in seq(length(lsec2))){
u<-paste0("https://www.gpw.pl/price-archive?fetch=0&type=1&instrument=&date=",format(as.Date(lsec2[i]),"%d-%m-%Y"),"&show_x=Show+results")
u2<-read_html(u)
u3<-html_table(u2)
  ...
  if(length(u3)>1){
    ...
    u4<-as_tibble(u3[[2]])
    u5<-t(u4$`Closing price`)
    u6<-as_tibble(cbind(lsec2[i],u5))
    colnames(u6)<-c("date",u4$Name %>% unlist)
    u7<-u6 %>% mutate_at(c(1),~as.Date(.)) %>% mutate_at(c(-1),~as.numeric(gsub(",","",.))) %>% select(c(1),all_of(ind$tickery))
    colnames(u7)<-c("date",ind$indeksy[as.numeric(na.omit(match(colnames(u7),ind$tickery)))])
    sector<-bind_rows(sector,u7)
    }
  }
print(lsec2[i])
flush.console()
...
save(ind,sector,file="C:/Users/j.morawski/Documents/ignacymorawski/ignacymorawski.github.io/data/sector.RData")
}
...
cat(paste0("- Ostatnia aktualizacja: ",Sys.time()))
```
...
urlbankier<-"https://www.bankier.pl/gielda/notowania/indeksy-gpw"
secbankier<-read_html(urlbankier) %>% html_table() 
secbankier2<-secbankier[[1]] %>% 
  as_tibble() %>% 
  select(c(1,2)) %>% 
  mutate_at(c(2),~as.numeric(gsub(intToUtf8(160),"",gsub(",",".",.)))) %>% 
  `colnames<-`(c("Name","Value")) %>% 
  filter(Name %in% ind$tickery) %>% 
  slice(match(ind$tickery,Name)) %>% 
  mutate(Name=ind$indeksy)

secbankier3<-secbankier2 %>% 
  pivot_wider(
    names_from="Name",
    values_from="Value"
  ) %>% 
  add_column(date=Sys.Date(),.before="WIG")

if (secbankier3$date==tail(sector$date,1)){
sector2<-tail(sector,51)  
} else {
sector2<-tail(sector,50) %>% add_row(secbankier3)
}
...
sectora<-sector2 %>% 
  ...
  mutate(ch1=100*.[[52]]/.[[51]]-100) %>% 
  mutate(ch7=100*.[[52]]/.[[47]]-100) %>%
  mutate(last=.[[52]]) %>%
  mutate(max=select(.,2:52) %>% apply(.,1,function(x) max(x))) %>% 
  select(Sector,ch1,ch7,last,max) %>% 
  mutate_at(c("Sector"),~factor(.,levels=unique(.))) %>% 
  mutate(ldate=Sys.Date()) %>% 
  mutate(sign1=ifelse(ch1<0,"","+")) %>% 
  mutate(sign2=ifelse(ch7<0,"","+"))
  
  sectora$color1<-rgb(0,0,0,maxColorValue = 255)
  sectora$color2<-rgb(0,0,0,maxColorValue = 255)
  sectora$color1[sectora$ch1<0]<-rgb(102-sectora$ch1[sectora$ch1<0]*5,0,0,maxColorValue = 255)
  sectora$color1[sectora$ch1>=0]<-rgb(0,102+sectora$ch1[sectora$ch1>=0]*5,0,maxColorValue = 255)
  sectora$color2[sectora$ch7<0]<-rgb(102-sectora$ch7[sectora$ch7<0]*5,0,0,maxColorValue = 255)
  sectora$color2[sectora$ch7>=0]<-rgb(0,102+sectora$ch7[sectora$ch7>=0]*5,0,maxColorValue = 255)
...
sector3<-sector2 %>% 
  pivot_longer(
   ...
   left_join(sectora,by="Sector") %>% 
  mutate_at(c("Sector"),~factor(.,levels=unique(.)))

if (sum(sector3$ch1)==0){
  sector3$color<-sector3$color2
} else {
  sector3$color<-sector3$color1
}

c<-1
for (i in ind$indeksy[c(1:16)]){
p1<-sector3 %>% filter(Sector==i)
clr1<-p1$color[1]

bp<-ggplot(p1,aes(x=date,y=Value))+
  geom_line(size=1,color="white")+
  geom_point(data=p1[3,],mapping=aes(ldate,last),size=2,color="white")+
  #geom_text(data=p1[3,],mapping=aes(date,max*1.04,label=paste0("1D: ",sign1,round(ch1,1),"%")),size=4,color="white",hjust=0,vjust=1,fontface="bold")+
  #geom_text(data=p1[3,],mapping=aes(ldate,max*1.04,label=paste0("7D: ",sign1,round(ch7,1),"%")),size=4,color="white",hjust=1,vjust=1,fontface="bold")+
  theme_minimal()+
  theme(panel.grid=element_blank())+
  theme(plot.background = element_rect(fill=clr1,color="gray"))+
  theme(panel.background = element_rect(fill=clr1,color=clr1))+
  theme(axis.text=element_text(color="white"))+
  theme(plot.title=element_text(size=15,hjust=0.5,color="white"))+
  theme(plot.subtitle=element_text(size=10,hjust=0.5,color="white"))+
  theme(plot.margin = margin(0.3,0.5,0.3,0,"cm"))+
  xlab("")+
  ylab("")+
  labs(
    title=i,
    subtitle=paste0("1 dzień: ",p1$sign1[3],round(p1$ch1[3],1),"%     ","7 dni: ",p1$sign2[3],round(p1$ch7[3],1),"%")
  )+
  scale_x_date(breaks=c(p1$date[1],p1$date[49]),labels=c(p1$date[1],p1$date[51]))

assign(paste0("bp",c),bp)
c<-c+1
}

plotCombine<-arrangeGrob(bp1,bp2,bp3,bp4,bp5,bp6,bp7,bp8,bp,bp10,bp11,bp12,bp13,bp14,bp15,bp16, nrow = 4)
grid.draw(plotCombine)
...
#### 3. Czym najmocniej handluje się na New Connect
...
#### 4. Które spółki przyciągają wyjątkową uwagę na rynku głównym GPW
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE,fig.width=11,fig.height=8}
...
#### 5. Które spółki przyciągają wyjątkową uwagę na New Connect
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE,fig.width=11,fig.height=8}
...
#### 6. Które spółki z rynku głównego są najbliżej rocznego maksimum
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE,fig.width=11,fig.height=8}
...
tab2<-tibble(spolka=tables0b$`Nazwa AD`,
              ...
              obroty=gsub(" ","", gsub(",",".",tables0b$`Obrót AD`))
) %>% 
...
n2a<-n1 %>% 
  ...
  arrange(desc(ratio),desc(obroty)) %>% 
  filter(turn>500000) %>% 
  head(30) %>% 
...
#### 7. Które spółki z New Connect są najbliżej rocznego maksimum
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE,fig.width=11,fig.height=8}
...
#### 8. Które spółki z rynku głównego są najbliżej rocznego minimum
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE,fig.width=11,fig.height=8}
...
#### 9. Które spółki z New Connect są najbliżej rocznego minimum
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE,fig.width=11,fig.height=8}
